...
```bash
node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
