...
- **Global scope**: `/workspaces/sveltekit-starter/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/sveltekit-starter/.claude/notes/`)
...
2. Glob `/workspaces/sveltekit-starter/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/sveltekit-starter/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/sveltekit-starter/.claude/notes/` — works in any directory
