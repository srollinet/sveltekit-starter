...
AGENT_SKILLS_AUDITOR=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-security-auditor --raw)
...
```
Task(
  prompt="Read /workspaces/sveltekit-starter/.claude/agents/gsd-security-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `/workspaces/sveltekit-starter/.claude/get-shit-done/templates/SECURITY.md`
...
```bash
node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update security threat verification"
```
