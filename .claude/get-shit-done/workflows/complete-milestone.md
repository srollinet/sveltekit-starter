...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
```bash
ARCHIVE=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `/workspaces/sveltekit-starter/.claude/get-shit-done/templates/retrospective.md`.
...
**Commit:**
```bash
node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(node "/workspaces/sveltekit-starter/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
